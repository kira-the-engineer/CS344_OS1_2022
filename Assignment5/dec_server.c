	...
	while(1){ //let's spin!
		...
		switch(pid){
			...
			case 0: { //if we actually spawned a child properly
				...
				char decryptmsg[MAX_BUFFER] = {0, }; //create string to store encrypted message
				...
				if(strcmp(recvbuf, DEC_CONFIRM_MSG) != 0){ //if the wrong client connects by accident
					...
					long decryptlen = strlen(decryptmsg); //get length of fully encrypted string
