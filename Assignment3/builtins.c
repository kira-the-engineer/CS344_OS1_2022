...
/* First we need to check if this is a comment or blank line */
	...
	if((strcmp(input[finalidx], "&")) == 0){ /* If & is found at last place in array */
	 ...
	 strcpy(currCMD->cmd, token); /* Copy token to cmd string in struct */
		...
		if((strcmp(input[i], "<")) == 0) { /* if char for input file is found */
		    token = (NULL, " ", saveptr); /* move pointer forward 1 */
		    strcpy(currCMD->inputFile, token);
		} 		
		else if((strcmp(input[i], ">")) == 0) { /* if char for output file is found */
		    token = (NULL, " ", saveptr); /* move pointer fwd 1 */
		    strcpy(currCMD->outputFile, token);
		}
		else { /* add to array of args */
		     if(aidx < 512) {
			 strcpy(currCMD->args[aidx], token); /* copy current token into idx in arg array */
			aidx++; /* increment idx */
		     }
		}
